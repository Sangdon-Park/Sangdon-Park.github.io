...
function renderCards(rows) {
  ...
  for (const student of rows) {
    ...
    add(heading, 'span', UI_EN ? 'Section ' + student.section : student.section + T('반'), 'student-card-section');
    ...
    add(meta, 'dd', student.current_problem ? student.current_problem + ' · ' + (student.current_language === 'java' ? 'Java' : student.current_language === 'c' ? 'C' : 'Python') : '—');
    add(meta, 'dt', T('최근 활동'));
...
function time(value){return new Date(value).toLocaleString(UI_DATE_LOCALE,{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});}
function render(){
 ...
 for(const student of rows){const tr=document.createElement('tr');const cell=text=>{const td=document.createElement('td');td.textContent=text;tr.append(td);return td;};cell(UI_EN?'Section '+student.section:student.section+T('반'));const person=cell(student.student_no);const name=document.createElement('strong');name.textContent=student.name;person.append(name);const progress=cell(student.solved+' / '+selectedTotal());const track=document.createElement('div');track.className='student-progress';const fill=document.createElement('div');fill.style.width=student.solved/selectedTotal()*100+'%';track.append(fill);progress.append(track);cell(student.attempts+T('회'));cell(student.current_problem+' · '+(student.current_language==='java'?'Java':student.current_language==='java'?'Java':language==='c'?'C':'Python'));cell(time(student.last_seen));const action=cell('');const button=document.createElement('button');button.textContent=T('이력 · 코드');button.onclick=()=>openStudent(student.id);action.append(button);$('student-rows').append(tr);}
...
async function refresh(){if(refreshing||!token)return;const activeToken=token;refreshing=true;$('admin-refresh').disabled=true;try{const data=await request('admin-dashboard');if(token!==activeToken)return;students=data.students;$('updated-at').textContent=T('최근 갱신 ')+new Date(data.updated_at).toLocaleTimeString(UI_DATE_LOCALE);$('admin-status').textContent='';render();}catch(error){$('admin-status').textContent=error.message;}finally{refreshing=false;$('admin-refresh').disabled=false;}}
...
async function openStudent(id){
 ...
 for(const p of selectedIds()){const box=document.createElement('div'),title=document.createElement('strong');title.textContent=p;box.append(title);for(const language of ['python','c','java']){const status=summary?.problems.find(item=>item.problem===p&&item.language===language),line=document.createElement('div');line.textContent=(language==='java'?'Java':language==='c'?'C':'Python')+': '+(status?.solved?T('✓ 통과'):status?T('진행 중'):T('미제출'));box.append(line);}$('detail-grid').append(box);}
