...
async function labRequest(action,body={},token=cloud.session?.token){
  ...
  const data=await response.json();if(!response.ok){if(response.status===401&&cloud.session)cloud.expired=true;const error=new Error(data.error||'서버 요청 실패');error.status=response.status;throw error;}return data;
}
function queueKey(){return 'dju-algolab-outbox:'+cloud.session.student.id;}
function saveQueue(){try{localStorage.setItem(queueKey(),JSON.stringify(cloud.queue));return true;}catch{cloudStatus('저장 공간 부족 · 답안 파일을 내려받아 보관하세요.',true);return false;}}
...
async function flushCloud(){
  if(cloud.flushing||!cloud.session||!cloud.queue.length)return;
  cloud.flushing=true;cloudStatus('서버에 저장 중…');
  try{
...
function cloudAccount(data){
  ...
  $('cloud-login').textContent=data.student.section+'반 · '+data.student.name;
...
function initCloudUI(){
  ...
  $('cloud-logout').onclick=async()=>{
    ...
    if(cloud.queue.length){cloudStatus('전송 대기 기록이 있습니다. 연결 복구 후 로그아웃하세요.',true);return;}
  ...
  $('cloud-import').onclick=()=>{
    ...
    $('import-description').textContent=`이 브라우저의 이전 답안 ${Object.keys(legacy.answers||{}).length}개를 ${cloud.session.student.section}반 ${cloud.session.student.name} 계정에 저장합니다. 이전 이름: ${legacy.student||'미입력'}. 본인의 답안인지 확인하세요. 같은 문제의 서버 초안은 가져온 코드로 바뀝니다.`;
  ...
  $('cloud-history').onclick=async()=>{
    $('history-dialog').showModal();$('history-list').textContent='불러오는 중…';
    try{const data=await labRequest('history');$('history-list').replaceChildren();for(const item of data.submissions){const row=document.createElement('p');row.textContent=`${item.problem} · ${item.language==='c'?'C':'Python'} · ${item.solved?'통과':`${item.passed}/${item.total}`} · ${new Date(item.created_at).toLocaleString('ko-KR')}${item.source==='legacy'?' · 기존 기록 가져옴':''}`;$('history-list').append(row);}if(!data.submissions.length)$('history-list').textContent='아직 제출 기록이 없습니다.';}catch(error){$('history-list').textContent=error.message;}
  };
  $('history-close').onclick=()=>$('history-dialog').close();
  setInterval(()=>{if(!cloud.session||document.hidden)return;flushCloud();labRequest('heartbeat',{problem:problems[index]?.id,language}).catch(()=>{});},30000);
