...
(()=>{
  ...
  const heading=document.createElement('h2');heading.textContent=UI_EN?'Reset this browser':T('이 브라우저 초기화');
  const description=document.createElement('p');
  description.textContent=UI_EN?'Remove all students’ saved answers, progress, sign-in information and editor settings from this browser. Server records remain available when the same student joins again. Python/C runtime downloads are kept.':T('이 브라우저에 남은 모든 학생의 답안·진도·접속 정보·편집기 설정을 지웁니다. 서버 기록은 유지되며 같은 학생 정보로 다시 접속하면 불러올 수 있습니다. Python/C 실행 환경 다운로드는 유지합니다.');
  const warning=document.createElement('p');warning.id='browser-reset-warning';warning.setAttribute('role','alert');
  const actions=document.createElement('div');actions.className='dialog-actions';
  const cancel=document.createElement('button');cancel.textContent=UI_EN?'Cancel':T('취소');cancel.onclick=()=>dialog.close();
  const confirm=document.createElement('button');confirm.id='confirm-browser-reset';confirm.textContent=UI_EN?'Clear local records':T('브라우저 기록 지우기');
  ...
  button.onclick=()=>{
    ...
    warning.textContent=pending?(UI_EN?`${pending} pending uploads will also be deleted. Cancel and wait for saving to finish if you need these answers.`:(T("아직 전송되지 않은 기록 ")+(pending)+T("건도 삭제됩니다. 필요한 답안이면 취소한 뒤 서버 저장 완료를 기다리세요."))):'';
    dialog.showModal();
  };
  confirm.onclick=async()=>{
    ...
    localStorage.setItem('dju-algolab-locale',UI_LOCALE);
