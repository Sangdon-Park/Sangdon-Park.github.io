...
(()=>{
  ...
  const tabs=el('div',null,'answer-tabs');tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label',T('답안 작성'));
  ...
  for(const [i,label] of [T('코드 작성'),T('풀이 설명')].entries()){
  ...
  const settings=el('details',null,'editor-settings');settings.append(el('summary',T('편집기 설정')));
  ...
  const execution=el('details',null,'execution-results');execution.append(el('summary',T('실행 결과')));results.before(execution);execution.append(results);results.querySelector('.panel-title').hidden=true;
  const contract=el('details',null,'function-guide');contract.append(el('summary',UI_EN?'Common coding help':T('공통 작성 도움말')));const instructions=document.getElementById('instructions');document.querySelector('aside').append(contract);contract.append(instructions);
  const resources=document.getElementById('chapter-resources'),resourceMenu=el('details',null,'resource-menu');resourceMenu.append(el('summary',T('강의 자료')));resources.before(resourceMenu);resourceMenu.append(resources);
