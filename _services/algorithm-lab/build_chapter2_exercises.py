...
WRITING_NUMBERS = [61, 65, 69, 71, 74, 78, 79, 80, 88, 99]
SHORT_QUESTIONS = {62: {'answer': '10,000회',
      'explanation': '0000부터 9999까지 빠짐없이 시도하면 10⁴ = 10,000회다. 정답을 찾는 즉시 멈추지 않고 끝까지 돌린 경우의 횟수다.',
      'task': '0~9로 이루어진 4자리 자물쇠를 0000부터 순서대로 모두 시도할 때, 마지막까지 갔다면 총 몇 회 시도한 것인지 쓰시오.'},
 63: {'answer': '10초',
      'explanation': '10,000 ÷ 1,000 = 10초다. 같은 속도로 8자리를 시도하면 1억 ÷ 1,000 = 10만 초, 약 28시간이 걸린다.',
      'task': '4자리 자물쇠를 초당 1,000회 시도할 수 있을 때, 최악의 경우 걸리는 시간을 쓰시오.'},
 64: {'answer': '알고리즘이 확인할 모든 후보의 집합',
      'explanation': '실행 시간은 후보 수와 후보당 처리 비용에 좌우된다. 코드를 작성하기 전에 두 값을 함께 살펴본다.',
      'task': "완전 탐색에서 '탐색 공간'이라는 말이 가리키는 것을 한 문장으로 쓰시오."},
 66: {'answer': '3가지',
      'explanation': '(1,1,2), (1,2,1), (2,1,1) 세 가지다. 삼중 반복문으로 216가지를 모두 만든 뒤 조건을 통과한 것만 세면 이 값이 나온다.',
      'task': '주사위 세 개를 던져 나온 눈의 합이 4가 되는 경우의 수를 쓰시오. (주사위는 서로 구별한다)'},
 67: {'answer': '약 50만 번 (1000 × 999 ÷ 2 = 499,500)',
      'explanation': 'n(n−1)/2 이므로 약 n²/2 이다. n이 10배가 되면 비교 횟수는 약 100배가 된다.',
      'task': '길이가 1,000인 리스트에서 서로 다른 두 원소의 모든 쌍을 검사하면 대략 몇 번 비교하게 되는지 쓰시오.'},
 68: {'answer': '정확히 2배',
      'explanation': '원래는 n(n−1)/2회, 변경 후에는 (n−1)n회이다. 바깥 반복문의 범위는 그대로이므로 비율은 정확히 2이다.',
      'task': 'n≥2이다. i는 0부터 n−2까지, j는 i+1부터 n−1까지 순회한다. j의 범위를 0부터 n−1까지로 바꾸면 검사 횟수는 몇 배가 되는가?'},
 70: {'answer': '반복문, 순열, 조합',
      'explanation': '변수가 적으면 중첩 반복문으로 충분하고, 순서를 정해야 하면 순열, 고르기만 하면 되면 조합을 쓴다. 부분집합이 필요하면 비트마스크를 함께 쓴다.',
      'task': '완전 탐색에서 후보 해를 만들어 낼 때 쓰이는 대표적인 도구 세 가지를 쓰시오.'},
 72: {'answer': '60가지',
      'explanation': '5P3 = 5 × 4 × 3 = 60이다. 누가 금인지 은인지가 달라지면 다른 결과이므로 순열로 센다.',
      'task': '5명 중에서 금메달, 은메달, 동메달을 받을 사람을 정하는 경우의 수를 쓰시오.'},
 73: {'answer': '10가지',
      'explanation': '5C2 = (5 × 4) ÷ 2 = 10이다. 짜장면과 짬뽕을 고르는 것과 짬뽕과 짜장면을 고르는 것은 같은 선택이므로 조합으로 센다.',
      'task': '5가지 음식 중 2가지를 골라 함께 먹는 경우의 수를 쓰시오.'},
 75: {'answer': 'n!',
      'explanation': '첫 자리에 n가지, 다음에 n−1가지 식으로 끝까지 곱한 값이다. 외판원 순회처럼 전체 순서를 정하는 문제에서 이 값이 그대로 탐색 공간이 된다.',
      'task': '서로 다른 n개를 하나도 남기지 않고 모두 나열하는 순열의 개수를 n에 대한 식으로 쓰시오.'},
 76: {'answer': '목표 깊이를 3에서 4로 바꾼다.',
      'explanation': '선택한 원소의 수가 r에 도달하면 하나의 조합이 완성된다. 이 원리는 Python과 C에서 같다.',
      'task': 'r개를 고르는 재귀 조합 탐색에서 선택 개수를 3개에서 4개로 바꾸면 종료 조건의 목표 깊이는 어떻게 바뀌는가?'},
 77: {'answer': '뽑은 것들의 자리를 서로 바꾸면 다른 경우가 되는가?',
      'explanation': '달라지면 순열, 같으면 조합이다. 메달 색이 바뀌면 다른 결과지만 세트 메뉴는 순서를 바꿔도 같은 메뉴다.',
      'task': '어떤 문제를 순열로 셀지 조합으로 셀지 판단할 때 스스로에게 던져야 할 질문을 한 문장으로 쓰시오.'},
 81: {'answer': 'n−m+1회',
      'explanation': '시작 위치는 0부터 n−m까지이며, 위치마다 한 번 비교한다.',
      'task': 'n≥m≥1이다. 텍스트의 가능한 시작 위치를 모두 검사하며 각 위치의 첫 글자에서 불일치한다면 총 문자 비교 횟수는 얼마인가?'},
 82: {'answer': '지금까지 검사한 쌍 중 차이가 가장 작은 쌍',
      'explanation': '아직 검사하지 않은 쌍까지 포함한 최적해는 아니다. 모든 쌍을 확인한 뒤 전체 최적해가 된다.',
      'task': '두 수의 차이를 비교하며 더 작은 차이를 만날 때 best 쌍을 갱신한다. 반복 도중 best가 담고 있는 것은 무엇인가?'},
 83: {'answer': '1,024개',
      'explanation': '2¹⁰ = 1,024다. 원소가 20개면 약 100만 개, 30개면 약 10억 개가 되어 곧 감당하기 어려워진다.',
      'task': '원소가 10개인 집합의 부분집합 개수를 쓰시오.'},
 84: {'answer': 'O(nᵏ), O(2ⁿ), O(n!)',
      'explanation': '각각 다항식, 지수, 팩토리얼 증가이다. 후보 한 개를 검사하는 비용은 별도로 곱한다.',
      'task': 'k가 고정된 중첩 반복, 모든 부분집합, 전체 순열의 후보 수 증가를 각각 빅오 표기로 쓰시오.'},
 85: {'answer': '120, 32, 3,125',
      'explanation': '5!=5×4×3×2×1, 2⁵=32, 5⁵=3,125이다.',
      'task': 'n=5일 때 n!, 2ⁿ, nⁿ의 값을 각각 쓰시오.'},
 86: {'answer': '1억 가지 (10⁸)',
      'explanation': '자리마다 10가지가 곱해진다. 초당 1만 번씩 시도한다면 1억 ÷ 1만 = 1만 초, 약 3시간이 걸린다.',
      'task': '각 자리가 0~9인 8자리 비밀번호의 경우의 수를 쓰시오.'},
 87: {'answer': '약 10억 개 (2³⁰ ≈ 1.07 × 10⁹)',
      'explanation': '원소가 20개일 때 약 100만이던 것이 30개에서 약 10억이 된다. 원소 10개가 늘었을 뿐인데 1,000배가 된 것이다.',
      'task': '원소가 30개인 집합의 부분집합 개수는 대략 얼마인지 쓰시오.'},
 89: {'answer': '2배',
      'explanation': '새 원소를 포함하는 경우와 포함하지 않는 경우로 갈라지므로 정확히 두 배가 된다. 이 두 배가 반복되어 2ⁿ이 된다.',
      'task': '부분집합을 만드는 문제에서 원소가 하나 늘어나면 경우의 수는 몇 배가 되는지 쓰시오.'},
 90: {'answer': '20',
      'explanation': '2²⁰=1,048,576이고 2²¹은 그 두 배이다. 실제 제한은 후보당 처리 비용에도 좌우된다.',
      'task': '후보를 최대 1,048,576개 검사할 수 있다. 부분집합 후보가 2ⁿ개이면 가능한 n의 최댓값은 얼마인가?'},
 91: {'answer': '10',
      'explanation': '10!=3,628,800은 제한 이하이고 11!=39,916,800은 제한을 넘는다.',
      'task': '후보를 최대 1,000만 개 검사할 수 있다. 전체 순열 후보가 n!개이면 가능한 n의 최댓값은 얼마인가?'},
 92: {'answer': '1개',
      'explanation': '2ⁿ⁺¹=2×2ⁿ이므로 같은 후보당 비용에서 원소를 하나 더 처리한다. 하드웨어 속도 향상만으로 지수 증가를 없앨 수는 없다.',
      'task': '컴퓨터가 2배 빨라지면 같은 시간 안에 검사할 수 있는 부분집합 문제의 원소 수 n은 얼마나 늘어나는가?'},
 93: {'answer': '백트래킹',
      'explanation': '두 문제 모두 규칙을 어기는 순간 그 갈래를 버리고 되돌아간다. 끝까지 만든 뒤 검사하는 완전 탐색보다 훨씬 적은 경우만 확인한다.',
      'task': 'N-Queens와 스도쿠는 완전 탐색에서 발전한 어떤 기법의 대표 응용인지 쓰시오.'},
 94: {'answer': '분할 정복',
      'explanation': '문제를 분할하고 작은 문제를 해결한 뒤 결합한다.',
      'task': '병합 정렬처럼 문제를 작은 문제로 나누어 각각 해결하고 결과를 합치는 기법의 이름을 쓰시오.'},
 95: {'answer': '단순 재귀 O(2ⁿ), 메모이제이션 O(n)',
      'explanation': '덧셈을 상수 시간으로 가정한다. 메모이제이션은 각 F(k)를 한 번만 계산한다.',
      'task': 'F(n)=F(n−1)+F(n−2)를 그대로 재귀 계산할 때와 메모이제이션할 때의 시간 복잡도 상한을 각각 쓰시오.'},
 96: {'answer': '부분 문제의 답을 저장하여 같은 계산에 재사용한다.',
      'explanation': '상향식 테이블 채우기 또는 하향식 메모이제이션으로 구현할 수 있다.',
      'task': '동적 계획법이 중복 부분 문제를 처리하는 핵심 방법을 한 문장으로 쓰시오.'},
 97: {'answer': '1,000,000번',
      'explanation': '100×100×100=1,000,000이다.',
      'task': '각각 정확히 n번 도는 삼중 반복문의 가장 안쪽 연산은 n=100일 때 몇 번 실행되는가?'},
 98: {'answer': '단일 반복, 중첩 반복, 조합 생성, 순열 생성, 비트마스크',
      'explanation': 'Python과 C 모두 같은 탐색 원리를 구현할 수 있다.',
      'task': '완전 탐색에서 쓰는 후보 생성 방식 다섯 가지를 쓰시오.'},
 100: {'answer': '앞에서 읽으나 뒤에서 읽으나 같은 문자열',
       'explanation': 'level과 noon이 예이다. 양 끝의 문자를 차례로 비교하여 확인할 수 있다.',
       'task': '회문(palindrome)이 무엇인지 한 문장으로 쓰시오.'}}
questions = [dict(p, kind='algorithm') if p['q'] in WRITING_NUMBERS else {
    'q': p['q'], 'kind': 'short', 'title': '개념 및 계산', 'section': p['section'], **SHORT_QUESTIONS[p['q']]
} for p in questions]
assert [p['q'] for p in questions] == list(range(61,101))
writing = [p for p in questions if p['kind'] == 'algorithm']
...
print(f'Built 10 algorithm-writing and 30 short-answer questions; {sum(len(p["tests"]) for p in writing)} Python reference tests passed.')
